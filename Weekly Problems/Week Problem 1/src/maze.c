...
int findPath(int x0,int y0){
    ...
    if(maze[x0][y0]=='|'){
        coordinates[++stackPointer].x=x0;   /*acrescenta o ponto a pseudo stack (coordinates)*/
        ...
        stackPointer--;                         /*remove virtualmente o ponto da stack*/
        return 0;
    }

    else if(maze[x0][y0]=='/'){
        coordinates[++stackPointer].x=x0;   
        coordinates[stackPointer].y=y0;
